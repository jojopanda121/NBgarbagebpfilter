...
const STAGE_CONFIG = {
  new:            { label: "新建",     color: "text-[#4B5A72]" },
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-[#EEF1F7] hover:bg-[#E5E9F4] rounded-lg"
        >
      ...
      {user && (
        <section>
          <h2 className="text-sm font-medium text-[#8E9BB0] uppercase tracking-wider mb-3">我的工作台</h2>
          {loadingPersonal ? (
            <div className="h-32 flex items-center justify-center bg-white rounded-xl border border-[#D8DCE8]">
          ...
          {personal && (
            <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
              <p className="text-sm font-medium text-[#4B5A72] mb-3 flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(STAGE_CONFIG).map(([key, cfg]) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2 px-3 py-2 bg-[#EEF1F7] rounded-lg">
                      <span className={`text-sm font-bold ${cfg.color}`}>{count}</span>
                      <span className="text-xs text-[#4B5A72]">{cfg.label}</span>
                    </div>
                  );
                })}
                {Object.values(personal.pipeline || {}).every(v => v === 0) && (
                  <span className="text-sm text-[#8E9BB0]">暂无项目数据</span>
                )}
              </div>

              {/* 即将到期跟进 */}
              {personal.upcoming_followups?.length > 0 && (
                <div className="mt-4 pt-3 border-t border-[#EEF1F7]">
                  <p className="text-xs text-[#8E9BB0] mb-2 flex items-center gap-1">
                    <Bell className="w-3 h-3" />
                    近期跟进提醒
                  </p>
                  {personal.upcoming_followups.map(f => (
                    <div key={f.id} className="flex items-center justify-between py-1.5 text-sm">
                      <span className="text-[#0F1C36]">{f.title || "BP项目"}</span>
          ...
          {/* 项目地理分布地图 */}
          {mapData && (
            <div className="bg-white border border-[#D8DCE8] rounded-xl p-4 mt-4">
              <p className="text-sm font-medium text-[#4B5A72] mb-3 flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                        selectedProvince === p.province
                          ...
                          : "bg-[#EEF1F7] border border-[#D8DCE8] text-[#4B5A72] hover:text-[#0F1C36] hover:bg-[#E5E9F4]"
                      }`}
                    >
                      {p.province} ({p.count})
                    </button>
              ...
              {selectedProvince && mapData?.details?.[selectedProvince] && (
                <div className="bg-[#EEF1F7] rounded-lg p-4 mt-3">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {mapData.details[selectedProvince].map((proj) => (
                      <div
                        ...
                        className="flex items-center justify-between py-1.5 border-b border-[#EEF1F7] last:border-0 cursor-pointer hover:bg-[#E5E9F4]/50 rounded px-2 -mx-2 transition-colors"
                      >
              ...
              {!mapData.provinces?.length && (
                <p className="text-sm text-[#8E9BB0] text-center py-6">暂无项目地理数据，在历史报告中为项目选择省份后将显示分布</p>
              )}
            </div>
          )}
        </section>
      ...
      {/* ── 平台公共数据（2.1）── */}
      <section>
        <h2 className="text-sm font-medium text-[#8E9BB0] uppercase tracking-wider mb-3">平台数据</h2>
        {loadingPlatform ? (
          <div className="h-48 flex items-center justify-center bg-white rounded-xl border border-[#D8DCE8]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4 text-center">
                <p className="text-3xl font-black text-[#0D2145]">
                  <AnimatedNumber value={platform.total_count} />
                </p>
                <p className="text-xs text-[#4B5A72] mt-1">累计分析 BP</p>
              </div>
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4 text-center">
                <p className="text-3xl font-black text-blue-400">
                  +<AnimatedNumber value={platform.weekly_new_display} />
                </p>
                <p className="text-xs text-[#4B5A72] mt-1">本周新增</p>
              </div>
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4 text-center">
                <p className="text-3xl font-black text-yellow-400">{platform.avg_score ?? "—"}</p>
                <p className="text-xs text-[#4B5A72] mt-1">平均评分</p>
              </div>
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4 text-center">
                <p className="text-3xl font-black text-emerald-400">{platform.top_score ?? "—"}</p>
                <p className="text-xs text-[#4B5A72] mt-1">历史最高分</p>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {/* 评级分布 */}
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
                <p className="text-sm font-medium text-[#0F1C36] mb-4">评级分布</p>
                  ...
                  {Object.entries(GRADE_COLORS).map(([grade, cfg]) => {
                    ...
                    return (
                      <div key={grade} className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-[#4B5A72] w-12 text-right">{pct}%</span>
                        <span className="text-xs text-[#8E9BB0] w-10 text-right">{count}份</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {/* 赛道热度 */}
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
                <p className="text-sm font-medium text-[#0F1C36] mb-4">赛道热度 Top 10</p>
                    ...
                    {platform.sector_top10.map((s, i) => {
                      ...
                      return (
                        <div key={s.sector} className="flex items-center gap-2">
                          <span className="text-xs text-[#8E9BB0] w-4">{i + 1}</span>
                          <span className="text-xs text-[#0F1C36] w-24 truncate">{s.sector}</span>
                          ...
                          <span className="text-xs text-[#8E9BB0] w-10 text-right">{s.count}</span>
                        </div>
                      );
                    })}
                  </div>
                ) : (
                  <p className="text-sm text-[#8E9BB0]">暂无数据</p>
                )}
              </div>
            </div>
            ...
            {platform.location_top10?.length > 0 && (
              <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
                <p className="text-sm font-medium text-[#0F1C36] mb-4 flex items-center gap-1.5">
                  <Globe className="w-4 h-4" />
                  地域分布
                </p>
                <div className="flex flex-wrap gap-2">
                  {platform.location_top10.map(l => (
                    <div key={l.location} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#EEF1F7] rounded-lg">
                      <span className="text-sm text-[#0F1C36]">{l.location}</span>
                      <span className="text-xs text-[#8E9BB0]">{l.count}</span>
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    return platform.monthly_trend.map(m => {
                      ...
                      return (
                        <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                          <span className="text-xs text-[#8E9BB0]">{m.count}</span>
                          <div
          ...
          <p className="text-[#8E9BB0] text-sm">暂无平台数据</p>
        )}
      </section>

      {/* ── 赛道情报（2.2）── */}
      <section>
        <h2 className="text-sm font-medium text-[#8E9BB0] uppercase tracking-wider mb-3">赛道情报</h2>
        <div className="bg-white border border-[#D8DCE8] rounded-xl overflow-hidden">
          {/* 赛道选择器 */}
          <div className="p-4 border-b border-[#EEF1F7] overflow-x-auto">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm whitespace-nowrap transition-colors ${
                    selectedSector === s
                      ...
                      : "bg-[#EEF1F7] text-[#4B5A72] hover:bg-[#E5E9F4]"
                  }`}
                >
                  {s}
                </button>
          ...
          <div className="p-4">
            {loadingSector ? (
              <div className="flex items-center gap-3 py-4">
                <Loader2 className="w-5 h-5 animate-spin text-blue-500" />
                <span className="text-[#4B5A72] text-sm">加载{selectedSector}赛道数据...</span>
              </div>
            ) : sectorData ? (
              <div>
                <p className="text-lg font-semibold text-[#0F1C36] mb-1">
                  {sectorData.sector}
                </p>
                ...
                {sectorData.grade_distribution && (
                  <div className="grid grid-cols-4 gap-2">
                    {["A", "B", "C", "D"].map(g => {
                      ...
                      return (
                        <div key={g} className="text-center py-3 bg-[#EEF1F7] rounded-lg">
                          <p className={`text-2xl font-bold ${cfg.text}`}>{count}</p>
                          <p className="text-xs text-[#8E9BB0] mt-1">{cfg.label}</p>
                        </div>
                      );
                    })}
                  </div>
              ...
              <p className="text-[#8E9BB0] text-sm py-2">暂无该赛道数据</p>
            )}
          </div>
        </div>
      </section>
...
function StatCard({ label, value, unit, icon }) {
  return (
    <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-2">{icon}<span className="text-xs text-[#8E9BB0]">{label}</span></div>
      <p className="text-2xl font-bold text-[#0D2145]">
        {typeof value === "number" ? value.toLocaleString() : value}
        {value !== "—" && <span className="text-sm font-normal text-[#8E9BB0] ml-1">{unit}</span>}
      </p>
    </div>
  );
}
...
function FileCountIcon() {
  return <BarChart2 className="w-5 h-5 text-[#4B5A72]" />;
}
