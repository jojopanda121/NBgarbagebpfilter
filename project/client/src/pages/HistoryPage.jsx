...
const STAGE_CONFIG = {
  new:            { label: "新建",     color: "bg-slate-100 text-[#526078] border-slate-300" },
...
function ProvinceSelector({ taskId, currentLocation, onUpdate }) {
  ...
  return (
    <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
      <MapPin className="w-3 h-3 text-[#526078] shrink-0" />
      <select
        value={currentLocation || ""}
        onChange={handleChange}
        disabled={saving}
        className="bg-[#E9EDF6] border border-[#CBD1E0] text-[#0C1A30] text-xs rounded px-1.5 py-0.5 focus:outline-none focus:border-blue-500 cursor-pointer hover:border-[#BFC5D6] transition-colors"
      >
...
export default function HistoryPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2 py-0.5 rounded text-xs bg-gray-500/20 text-[#2D3D54]">
            {status}
          </span>
        );
    }
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          disabled={refreshing}
          className="flex items-center gap-2 px-3 py-1.5 text-sm bg-[#E9EDF6] hover:bg-[#DDE3F0] rounded-lg transition-colors"
        >
      ...
      {tasks.length > 0 && (
        <div className="flex flex-wrap items-center gap-3 mb-5">
          ...
          <div className="flex items-center gap-1.5 text-sm">
            <ArrowUpDown className="w-4 h-4 text-[#526078]" />
            <button
              onClick={() => setSortBy("time")}
              className={`px-3 py-1 rounded-full border transition-colors ${
                sortBy === "time"
                  ? "bg-blue-500/20 text-blue-400 border-blue-500/40"
                  : "bg-[#E9EDF6] text-[#2D3D54] border-[#CBD1E0] hover:border-[#A8B0C8]"
              }`}
            >
              按时间
            </button>
          ...
          <div className="flex items-center gap-1.5 text-sm ml-auto">
            <Filter className="w-4 h-4 text-[#526078]" />
            <select
              value={filterIndustry}
              onChange={(e) => setFilterIndustry(e.target.value)}
              className="bg-[#E9EDF6] border border-[#CBD1E0] text-[#0C1A30] text-sm rounded-lg px-3 py-1.5 focus:outline-none focus:border-blue-500"
            >
      ...
      {tasks.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl border border-[#CBD1E0]">
          <FileText className="w-12 h-12 text-[#526078] mx-auto mb-4" />
          <p className="text-[#2D3D54] mb-4">暂无分析记录</p>
          <button
            onClick={() => navigate("/app/dashboard")}
            className="px-4 py-2 bg-[#1749C9] hover:bg-[#0A1F3D] rounded-lg font-medium transition-colors"
          >
            开始第一次分析
          </button>
        </div>
      ) : filteredTasks.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-[#CBD1E0]">
          <p className="text-[#2D3D54]">当前筛选条件下无匹配报告</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filteredTasks.map((task) => (
            <div
              ...
              className={`p-4 bg-white border border-[#CBD1E0] rounded-xl transition-colors ${
                task.status === "complete" || task.status === "running"
                  ? "cursor-pointer hover:border-[#CBD1E0] hover:bg-[#E9EDF6]"
                  : "opacity-75"
              }`}
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  {task.status === "complete" && task.total_score != null ? (
                    ...
                    <div className="w-10 h-10 bg-[#E9EDF6] rounded-lg flex items-center justify-center shrink-0">
                      <FileText className="w-5 h-5 text-[#2D3D54]" />
                    </div>
                  )}
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap text-sm text-[#526078]">
                      {task.archive_number && (
                        <span className="px-2 py-0.5 rounded text-xs bg-[#DDE3F0] text-[#0C1A30] font-mono font-semibold">
                          {task.archive_number}
                        </span>
                      )}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {task.status === "complete" && task.project_stage && task.project_stage !== "new" && (
                    <span className={`px-2 py-0.5 rounded border text-xs font-medium ${
                      STAGE_CONFIG[task.project_stage]?.color || "bg-slate-500/20 text-[#2D3D54] border-slate-500/30"
                    }`}>
                  ...
                  {task.status === "complete" && (
                    <button
                      ...
                      className="p-1 text-[#526078] hover:text-red-400 transition-colors"
                      title="删除报告"
                    >
                  ...
                  {(task.status === "complete" || task.status === "running") && (
                    <ChevronRight className="w-5 h-5 text-[#526078]" />
                  )}
                </div>
              </div>
              ...
              {task.status === "running" && (
                <div className="mt-3">
                  <div className="flex items-center justify-between text-xs text-[#526078] mb-1">
                    <span>{task.message}</span>
                    <span>{task.percentage || 0}%</span>
                  </div>
                  <div className="h-1 bg-[#DDE3F0] rounded-full overflow-hidden">
                    <div
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white border border-[#CBD1E0] rounded-xl p-6 max-w-sm mx-4 shadow-2xl">
            ...
            <p className="text-[#2D3D54] text-sm mb-6">
              删除后该报告将从您的列表中移除，<strong className="text-red-400">此操作不可恢复</strong>。确定要继续吗？
            </p>
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="px-4 py-2 text-sm bg-[#E9EDF6] hover:bg-[#DDE3F0] rounded-lg transition-colors"
              >
                取消
              </button>
              <button
