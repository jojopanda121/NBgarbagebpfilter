...
// ============================================================
// buildPrompt — 动态 Prompt 构建器
//
// "铁打的评判标准（静态规则），流水的 BP 项目（动态变量）"
//
// 前置依赖: Agent A 已提取 company_name, industry, bp_claims 等基本信息
// 此函数在 Phase 2 结构化评分时使用，将动态上下文注入到静态评判规则中
// ============================================================

/**
 * 构建发送给 MiniMax 的结构化评分 Prompt
 *
 * @param {Object} projectContext - Agent A 提取的项目上下文
 * @param {string} projectContext.company_name - 公司名称
 * @param {string} projectContext.industry - 细分赛道
 * @param {string} projectContext.product_name - 产品名称
 * @param {string} projectContext.bp_claims_product - BP 中的产品竞争力声明
 * @param {string} projectContext.Business_Model - 商业模式描述
 * @param {string} projectContext.Growth_Engine - 增长引擎描述
 * @param {string} projectContext.Network_Effect - 网络效应描述
...
你是一个融合了资深行业专家（10年+行业经验）和顶级投资分析师（曾管理10亿+美元基金）的 AI 研究员。你对中国各行业市场格局、技术发展趋势、一级市场投资逻辑有深刻理解。
...
你只负责输出客观的枚举值和绝对数值，最终评分由 JS 代码定量计算，你不需要给总分。

**任务A：五维度数据输出（基于核查后的真实数据）**

*第一维度：时机与天花板*
- 不依赖 BP 声称，去客观检索 ${industry} 赛道的真实市场规模（TAM）和 CAGR
- TAM 必须统一转换为百万人民币（纯数字输出，如 1 亿输出 100，若 BP 用美元需按 1:7.2 换算）
- CAGR 输出为百分比的数字部分（如 25 表示 25%）
- 参考核查报告中 category=market 的核查结论
- 评估入场时机（行业周期位置、竞争格局）

*第二维度：产品与壁垒*
- 检索 ${industry} 行业内 ${product_name} 的真实竞品及行业排名
- 输出 Competitor_Rank_Score（1-10 整数）：
  8-10分 = 行业 Top 5 且极难复制（技术壁垒/网络效应/独占资源）
  4-7分 = 腰部或细分第一（有差异化但可追赶）
  1-3分 = 红海同质化跟风者（无明显差异化）
- 基于核查的技术真实性重新评定 TRL 级别
- 竞品分析（点名 3-5 个主要竞品，注明融资/估值情况）

*第三维度：资本效率与规模效应*
- 基于顶级 VC 框架评估 ${industry} 赛道的宏观属性（不要求该公司的微观财务数据）
- 输出两个 1-10 分的整数：
  子维度1: Industry_Capital_Score（行业资本效率）
  - [10-8分] 轻资产：纯软件/SaaS/内容平台，客户预付/年付模式，负营运资本
  - [7-5分] 中等：有一定库存或供应链投入但周转快，LTV/CAC 健康
  - [4-1分] 重资产：制造/基建/硬件，资金沉淀在固定资产，高 CapEx
  子维度2: Industry_Scale_Score（行业规模效应）
  - [10-8分] 赢家通吃：双边网络效应或绝对规模经济
  - [7-5分] 强者恒强：单边网络效应、数据飞轮、品牌壁垒
  - [4-1分] 规模不经济：人力密集型，收入与人力成本呈线性关系
  【重要】若数据不足，Industry_Capital_Score 和 Industry_Scale_Score 各默认 5 分（中性分），禁止输出 0 或 null

*第四维度：团队基因*
- 仅提取核心创始人在 ${industry} 赛道的直接相关从业经验年数 Founder_Exp_Years
- 参考核查报告中 category=team 的核查结论验证背景真实性
- 不再评估股权结构（已从评分模型中移除）

*第五维度：外部风险与交易条件（乘数）*
- 参考核查报告中 category=valuation 的核查结论
- 政策合规风险（列举相关法规，输出 Policy_Risk: 0-1）
- 基于核查后的真实收入数据重新计算估值溢价倍数 Valuation_Gap
...
【缺失数据处理原则】
- 若 BP 未披露某维度的关键数据，且 AI 知识库中亦无足够的行业对比数据，禁止给出极端值
- TAM_Million_RMB 缺失时参考同赛道中位数，CAGR 缺失时使用行业均值
- Competitor_Rank_Score 缺失时默认 5（行业中游），Founder_Exp_Years 缺失时默认 3
- Industry_Capital_Score 和 Industry_Scale_Score 缺失时各默认 5，不得低于 1 或高于 10
- 在 finding 中注明"由于数据缺失，采用行业中立基准"

【重要输出要求】只输出纯 JSON，不要任何 markdown 代码块或其他文字：
{
  "one_line_summary": "${industry}赛道，A轮早期阶段，技术路线清晰但市场数据夸大严重",
  "validated_data": {
    "TAM_Million_RMB": 5000,
    "CAGR": 20,
    "TRL": 6,
    "Competitor_Rank_Score": 6,
    "Industry_Capital_Score": 7,
  ...
  "dimension_analysis": {
    "timing_ceiling": {
      ...
      "finding": "深度分析：基于核查报告，${industry}赛道真实TAM约XX百万人民币（BP声称XX，核查结论：夸大X倍）。行业CAGR实际为XX%。",
      "bp_claim": "BP声称市场规模XX亿，年增速XX%",
      "ai_finding": "核查结论：真实市场规模约XX百万人民币（参考：XX报告），增速约XX%"
    },
    "product_moat": {
      "label": "产品与壁垒",
      "subtitle": "TRL + 竞品排名",
      "finding": "竞品排名评估：经检索${industry}行业竞品，${product_name}排名为Competitor_Rank_Score=X。主要竞品：①XX ②XX ③XX。TRL评级为X级。",
      "bp_claim": "BP声称技术领先同行2-3年",
      "ai_finding": "核查结论：该技术路线已有XX家公司布局..."
    },
    "business_validation": {
      "label": "资本效率与规模效应",
      ...
      "Industry_Capital_Score": 7,
      "Industry_Scale_Score": 7,
      "finding": "基于${industry}赛道宏观属性评估：资本效率=X（原因：XX），规模效应=X（原因：XX）。",
      "bp_claim": "BP声称的商业模式：${Business_Model}",
      "ai_finding": "核查结论：该赛道资本效率属于[高/中/低]水平..."
    },
    "team": {
      "label": "团队基因",
      "subtitle": "创始人赛道经验年数",
      "finding": "核查结论：创始人在${industry}赛道直接从业经验X年，[高度匹配/基本匹配/存在短板]。",
      "bp_claim": "BP声称创始人有XX年行业经验",
      "ai_finding": "核查结论：该背景[可信/存疑/需核实]..."
    },
    "external_risk": {
      "label": "外部风险",
      "subtitle": "政策风险 + 估值溢价折扣",
      "finding": "政策风险：涉及[XX监管领域]，风险等级[低/中/高]。估值溢价倍数=XX。",
      "bp_claim": "BP声称估值XX亿",
      "ai_finding": "核查结论：可比公司估值水平：①XX ②XX。行业平均PS=XX。",
      "multiplier": 0.9
    }
  },
  "risk_flags": [
  ...
  "valuation_comparison": {
    ...
    "comparable_companies": [
      "可比公司1（上市/融资阶段）：估值倍数 Xx",
      "可比公司2：估值倍数 Xx"
    ],
    "data_source": "MiniMax AI 知识库综合分析",
    "analysis": "估值合理性分析..."
  },
  "validation_notes": {
    "TAM_Million_RMB": "说明TAM换算过程和数据来源",
    "CAGR": "说明CAGR校准依据",
    "TRL": "说明TRL评级依据",
    "Competitor_Rank_Score": "说明竞品排名评估依据",
    "Industry_Capital_Score": "说明资本效率评估依据",
    "Industry_Scale_Score": "说明规模效应评估依据",
    "Founder_Exp_Years": "说明经验年数核查结果",
    "Valuation_Gap": "说明估值溢价计算过程"
  }
}
...
- validated_data 中 TAM_Million_RMB 必须是百万人民币单位的数字（1亿=100）
- comparable_companies 要点名具体公司和估值数据
- 所有数值字段必须是数字类型，validated_data 中所有字段不能为 null
- one_line_summary 格式：项目定位（赛道/技术）+ 当前所处阶段 + 核心亮点或致命缺陷
- Industry_Capital_Score 和 Industry_Scale_Score 必须是1-10之间的整数，缺失时默认5
- Competitor_Rank_Score 必须是1-10之间的整数
- Founder_Exp_Years 必须是数字（年）`;

  return dynamicContext + staticRules;
}

// 保留一个静态引用供普通重试时使用（不含动态上下文时的兜底）
const EXPERT_JUDGE_STRUCTURAL_PROMPT = buildStructuralPrompt({});
...
- 数值字段必须是数字类型，不得为null
- TAM_Million_RMB 必须是百万人民币单位的纯数字（1亿=100）
- Industry_Capital_Score和Industry_Scale_Score必须是1-10的整数，无数据默认5
- Competitor_Rank_Score必须是1-10的整数，无数据默认5
- Founder_Exp_Years必须是数字（年），无数据默认3
...
只输出以下结构的JSON，不加任何markdown或解释：
{
  ...
  "dimension_analysis": {
    "timing_ceiling": {
      "label": "时机与天花板", "subtitle": "TAM（百万人民币）+CAGR",
  ...
  "validation_notes": {
    "TAM_Million_RMB": "说明", "CAGR": "说明", "TRL": "说明",
    "Competitor_Rank_Score": "说明",
    "Industry_Capital_Score": "说明", "Industry_Scale_Score": "说明",
    "Founder_Exp_Years": "说明", "Valuation_Gap": "说明"
  }
}`;
...
async function runAgentBWithBatching(extractedData, bpText, onProgress) {
  ...
  // 动态上下文注入：将公司基本信息拼接到核查 Prompt 前面
  const bpContext = `请对处于 ${extractedData.industry || "未知"} 赛道的 ${extractedData.company_name || "未知公司"} 进行核查。产品：${extractedData.product_name || "未知"}。针对其 BP 中的声明，执行以下检索和核查规则`;
  ...
  // 使用 buildStructuralPrompt 动态注入项目上下文到评判规则中
  const dynamicPrompt = buildStructuralPrompt({
    company_name: extractedData.company_name,
    industry: extractedData.industry,
    product_name: extractedData.product_name,
    bp_claims_product: extractedData.bp_claims_product,
...
async function runPipelineBackground(taskId, bpText) {
  ...
  // ── 评分计算（v4.0 新 Schema 兼容层）──
  // 处理 AI 可能输出旧字段名的情况，统一映射到新 Schema
  ...
  const scoringInput = {
    ...
    Competitor_Rank_Score: rawScoringData.Competitor_Rank_Score ?? rawScoringData.SC ?? 5,
    // S3: 资本效率与规模效应
    Industry_Capital_Score: rawScoringData.Industry_Capital_Score ?? rawScoringData.Capital_Efficiency ?? 5,
    Industry_Scale_Score: rawScoringData.Industry_Scale_Score ?? rawScoringData.Scale_Effect ?? 5,
    // S4: 团队基因
    Founder_Exp_Years: rawScoringData.Founder_Exp_Years ?? rawScoringData.Exp ?? 3,
    // V5: 外部风险
  ...
  const verdict = {
    ...
    dimensions: {
      timing_ceiling: {
        ...
        weight: scoringResult.dimensions.timing_ceiling.weight,
        ...
        inputs: scoringResult.dimensions.timing_ceiling.inputs,
      },
      product_moat: {
        ...
        weight: scoringResult.dimensions.product_moat.weight,
        ...
        inputs: scoringResult.dimensions.product_moat.inputs,
      },
      business_validation: {
        ...
        weight: scoringResult.dimensions.business_validation.weight,
        ...
        inputs: scoringResult.dimensions.business_validation.inputs,
      },
      team: {
        ...
        weight: scoringResult.dimensions.team.weight,
        ...
        inputs: scoringResult.dimensions.team.inputs,
      },
      external_risk: {
        ...
        weight: scoringResult.dimensions.external_risk.weight,
        ...
        inputs: scoringResult.dimensions.external_risk.inputs,
      },
    },
  ...
  const deepResearchInput = [
    ...
    `\n\n【项目基本信息】\n公司：${extractedData.company_name || "未知"}，赛道：${extractedData.industry || "未知"}，产品：${extractedData.product_name || "未知"}`,
    `\n\n【AI专家深度分析结果（v4.0 新评分体系）】\n${JSON.stringify(
      {
        scoring: {
          total_score: scoringResult.total_score,
          grade: scoringResult.grade,
