...
// 每批处理的声明数量上限；调小可降低单批输出量，调大可减少 API 调用次数
const CLAIM_BATCH_SIZE = parseInt(process.env.CLAIM_BATCH_SIZE, 10) || 7;
...
// ── Agent B 结构化分析 Prompt（仅输出 validated_data + dimension_analysis 等固定量字段） ──
// 声明逐条核查（claim_verdicts）由 CLAIM_VERDICT_BATCH_PROMPT 分批并发处理，
// 从而彻底规避单次输出量随声明数线性增长导致的 max_tokens 超载问题。
...
你的使命：基于你的专业知识库，对这份商业计划书（BP）进行严格的深度尽职调查。你将收到：
1. 【BP提取数据】— AI从BP中提取的结构化数据和关键声明
2. 【BP原文节选】— 商业计划书原文

⚠️ 重要分工说明：声明逐条核查（claim_verdicts）已由独立批处理模块并行处理，本次请【不要输出 claim_verdicts 字段】，只专注于以下两项任务。

**任务B：五维度深度评分（请用 DeepThink 模式，逐步推理）**
...
【重要输出要求】只输出纯 JSON，不要任何 markdown 代码块或其他文字：
{
  ...
  "dimension_analysis": {
    "timing_ceiling": {
      ...
      "label": "时机与天花板",
      "subtitle": "市场规模 + 入场时机",
      "finding": "深度分析：真实TAM约XX亿元（BP声称XX亿，夸大X倍）。行业CAGR实际为XX%，主要驱动因素为：①... ②... ③...。入场时机评估：该行业目前处于[早期/高速增长/成熟]阶段...",
      "bp_claim": "BP声称市场规模XX亿，年增速XX%",
      "ai_finding": "AI研究发现：真实市场规模约XX亿（参考：IDC报告/艾瑞咨询/可比市场数据），增速约XX%..."
    },
    "product_moat": {
      "score": 65,
      "label": "产品与壁垒",
      ...
      "ai_finding": "AI研究发现：该技术路线已有XX家公司布局，其中头部竞品为XX..."
    },
    "business_validation": {
      "score": 60,
      "label": "商业验证与效率",
      "subtitle": "商业模式 + 增长 + 财务",
      "finding": "单位经济验证：LTV/CAC=XX（行业平均约为X-X），[合理/偏低/明显偏低]。毛利率验证：BP声称XX%，同类型公司（XX、XX）实际毛利率约XX-XX%...",
  ...
  "valuation_comparison": {
    ...
    "analysis": "BP要求估值20x PS，而行业可比公司中位数为10x PS，溢价约100%。建议将估值压缩至10-12x PS区间。"
  },
  "validation_notes": {
...
// ── 声明批量核查 Prompt（每次只处理 CLAIM_BATCH_SIZE 条声明，输出量受批大小严格上界） ──
const CLAIM_VERDICT_BATCH_PROMPT = `你是一个融合了资深行业专家（10年+行业经验）和顶级投资分析师的 AI 研究员，擅长深度核查创业公司商业计划书中关键声明的真实性。

你将收到一批来自商业计划书（BP）的关键声明（每批不超过 ${CLAIM_BATCH_SIZE} 条），以及BP的基础信息和原文节选。

请对【本批声明】中的每一条声明进行深度核查：
1. 基于你的专业知识研究该领域的真实情况（引用具体数据：市场报告、可比公司数据、技术基准等）
2. 判断声明真实性：
...
【重要输出要求】只输出纯 JSON，不要任何 markdown 代码块或其他文字，格式如下：
{
  "claim_verdicts": [
    {
      "category": "market",
      "original_claim": "声明原文",
      "bp_claim": "BP具体声称：...",
      "ai_research": "AI基于知识库的深度研究：该市场2024年实际规模约XX亿元，主要依据：（1）IDC/艾瑞/Frost&Sullivan等机构报告；（2）可比市场参考：XX市场规模XX亿；（3）增速驱动因素分析...",
      "verdict": "夸大",
...
每条声明的 ai_research 字段必须有实质性的行业数据对比（不少于50字），不能只是重复声明内容。`;
...
// ============================================================
// Agent B 分批处理引擎
// 彻底解决"不定量 claims → 单次输出超载 → JSON 截断"问题
//
// 设计原理：
//   Agent B 的输出由两类字段组成：
//   (1) 固定量字段：validated_data / dimension_analysis / risk_flags /
//       strengths / conflicts / valuation_comparison / validation_notes
//       ── 总输出约 5000-8000 tokens，与 claims 数量无关，一次 DeepThink 调用即可
//   (2) 可变量字段：claim_verdicts（随 claims 数量线性增长）
//       ── 25 条 × 700 tokens/条 ≈ 17500 tokens，单次调用必然超载
//
// 解法：将 (1) 和 (2) 分离，(2) 按 CLAIM_BATCH_SIZE 切批，各批并发调用，最后合并。
// 无论 claims 有多少条，每次 LLM 调用的输出量都被严格上界所控制。
// ============================================================

/**
 * 单批声明核查：对一批 claims 调用 LLM（普通模式），返回 claim_verdicts 数组。
 * 失败时自动重试一次；两次均失败则返回存根（保证数量完整，不丢弃任何声明）。
 */
async function callClaimVerdictBatch(claims, extractedData, bpText, batchIdx) {
  const batchInput = [
    `【BP基础信息】`,
    `公司：${extractedData.company_name || "未知"}，行业：${extractedData.industry || "未知"}`,
    `核心指标：TAM=${extractedData.TAM}亿, CAGR=${extractedData.CAGR}%, TRL=${extractedData.TRL}, Margin=${extractedData.Margin}, LTV/CAC=${extractedData.Ratio}`,
    ``,
    `【本批声明（第${batchIdx + 1}批，共${claims.length}条，请逐条核查，不得遗漏）】`,
    JSON.stringify(claims, null, 2),
    ``,
    `【BP原文节选（前8000字，供参考上下文）】`,
    bpText.slice(0, 8000),
  ].join("\n");

  // 单批最大输出：CLAIM_BATCH_SIZE × ~700 tokens ≈ 4900，留出余量设为 6000
  let raw = await callLLM(CLAIM_VERDICT_BATCH_PROMPT, batchInput, 6000);
  let parsed = extractJson(raw);

  if (!parsed || !Array.isArray(parsed.claim_verdicts)) {
    console.warn(`[AgentB-Batch${batchIdx + 1}] JSON 解析失败，重试...`);
    raw = await callLLM(
      CLAIM_VERDICT_BATCH_PROMPT + '\n\n【紧急】只输出合法 JSON，格式必须是 {"claim_verdicts": [...]}，从 { 开始到 } 结束。',
      batchInput,
      6000
    );
  ...
  if (parsed && Array.isArray(parsed.claim_verdicts)) {
    console.log(`  → Batch ${batchIdx + 1}: 完成 ${parsed.claim_verdicts.length} 条声明核查`);
    return parsed.claim_verdicts;
  }

  // 两次均失败：返回存根，保证原始声明全量保留，不静默丢弃
  console.error(`[AgentB-Batch${batchIdx + 1}] 两次解析均失败，使用存根占位`);
  return claims.map((c) => ({
    category: c.category,
    original_claim: c.claim,
    bp_claim: c.claim,
    ai_research: "该批次 LLM 响应解析失败，无法完成深度核查",
    verdict: "存疑",
    diff: "解析失败，无法量化差异",
    severity: "中",
    score_impact: "未知",
 ...
 *
 * 并发启动两类任务：
 *   - 结构化分析（1次 DeepThink 调用）→ validated_data + dimension_analysis + ...
 *   - 声明批次（N次普通调用，p-limit 控制并发）→ claim_verdicts[batch_0..N]
 * 全部完成后合并，返回与原 EXPERT_JUDGE_PROMPT 完全兼容的 validatedData 对象。
 */
async function runAgentBWithBatching(extractedData, bpText) {
  const claims = extractedData.key_claims || [];
  const batches = chunkArray(claims, CLAIM_BATCH_SIZE);
  console.log(
    `  → Agent B 分批模式：${claims.length} 条声明 / ${CLAIM_BATCH_SIZE} 条/批 = ${batches.length} 批`
  );
  ...
  // 声明批次并发限制（防止同时打出过多请求触发 API QPS 限速）
  const limit = pLimit(2);

  // 并发启动：结构化分析（DeepThink，固定量输出）+ 所有声明批次（普通模式，批量输出）
  const [structuralResult, ...batchResults] = await Promise.all([
    callLLMWithThinking(EXPERT_JUDGE_STRUCTURAL_PROMPT, structuralInput, 10000, 6000),
    ...batches.map((batch, idx) =>
      limit(() => callClaimVerdictBatch(batch, extractedData, bpText, idx))
    ),
  ]);

  // 解析结构化分析结果
  let structuredData = extractJson(structuralResult.text);
  if (!structuredData || !structuredData.validated_data) {
    console.warn("[AgentB] 结构化分析解析失败，降级为普通模式重试...");
    const retryRaw = await callLLM(
      EXPERT_JUDGE_STRUCTURAL_PROMPT + "\n\n【紧急】只输出合法 JSON，从 { 开始到 } 结束，不要任何其他内容。",
      structuralInput,
      10000
    );
    structuredData = extractJson(retryRaw);
  }

  if (!structuredData || !structuredData.validated_data) {
    throw new Error("Agent B 结构化分析两次均失败");
  }

  // 合并所有批次的声明核查结果（flat 展开，保持原始顺序）
  structuredData.claim_verdicts = batchResults.flat();
  console.log(
    `  → Agent B 完成：结构化分析 ✓ + ${structuredData.claim_verdicts.length}/${claims.length} 条声明核查 ✓`
  );

  return { thinking: structuralResult.thinking || "", data: structuredData };
}
...
app.post("/api/analyze", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    // ── 第2步: AI专家深度研究与评分（分批处理模式，彻底解决 token 超载）──
    // 结构化分析（DeepThink）与所有声明批次并发执行，最后合并，
    // 无论 claims 数量多少，每次 LLM 调用的输出量都受 CLAIM_BATCH_SIZE 严格控制。
    const claimsCount = (extractedData.key_claims || []).length;
    const batchCount = Math.ceil(claimsCount / CLAIM_BATCH_SIZE);
    console.log(`[2/2] AI专家深度研究: 结构化分析(DeepThink) + ${batchCount}批声明核查 并发执行...`);
    sendSSE({
      type: "progress",
      stage: "ai_research",
      percentage: 32,
      message: `AI深度研究启动（${claimsCount}条声明拆分为${batchCount}批并发处理，预计3-6分钟）...`,
    });
    ...
    try {
      const agentBResult = await runAgentBWithBatching(extractedData, bpText);
      thinking = agentBResult.thinking;
      validatedData = agentBResult.data;
    } catch (agentBErr) {
      console.error("[2/2] Agent B 分批处理失败:", agentBErr.message);
