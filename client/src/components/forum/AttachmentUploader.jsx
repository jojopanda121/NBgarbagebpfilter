...
export default function AttachmentUploader({ value = [], onChange, scope = "post", disabled = false }) {
  ...
  async function handleSelect(e) {
    ...
    try {
      for (const file of picked) {
        ...
        if (!isImage && !isFile) { setError(`不支持的类型：${file.name}`); continue; }
        if (commentMode && !isImage) { setError("评论仅支持图片"); continue; }
        if (isImage && file.size > IMAGE_MAX) { setError(`图片超过 5MB：${file.name}`); continue; }
        if (isFile && file.size > FILE_MAX) { setError(`文档超过 20MB：${file.name}`); continue; }
        if (isImage && imgCount >= maxImages) { setError(`最多 ${maxImages} 张图片`); continue; }
        if (isFile && fileCount >= maxFiles) { setError(`最多 ${maxFiles} 个文档`); continue; }

        try {
          const att = await forumApi.uploadAttachment(file);
          next.push(att);
          if (att.kind === "image") imgCount += 1; else fileCount += 1;
        } catch (uErr) {
  ...
  return (
    <div className="space-y-2">
      <input ref={inputRef} type="file" accept={accept} multiple={!commentMode} className="hidden" onChange={handleSelect} />

      <div className="flex items-center gap-2 flex-wrap">
        <button type="button" onClick={() => inputRef.current?.click()} disabled={!canAdd}
          ...
          {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : (commentMode ? <ImagePlus className="w-4 h-4" /> : <Paperclip className="w-4 h-4" />)}
          {commentMode ? "图片" : "图片 / 文档"}
        </button>
        {!commentMode && (
          <span className="text-[11px] text-[#8E9BB0]">最多 9 张图 + 3 个文档，图≤5MB / 文档≤20MB</span>
        )}
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {images.map((a) => (
            <div key={a.url} className="relative group">
              <img src={a.url} alt={a.name} className="w-16 h-16 object-cover rounded-lg border border-[#D8DCE8]" />
      ...
      {files.length > 0 && (
        <div className="space-y-1">
          {files.map((a) => (
            <div key={a.url} className="flex items-center gap-2 text-xs bg-[#EEF1F7] border border-[#D8DCE8] rounded-lg px-2.5 py-1.5">
              ...
              <button type="button" onClick={() => remove(a.url)} className="text-[#8E9BB0] hover:text-rose-500">
