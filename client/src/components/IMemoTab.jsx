...
export default function IMemoTab({ taskId }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-3 text-slate-400">生成投资备忘录...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* 操作栏 */}
      <div className="flex items-center justify-between p-3 bg-slate-900 border border-white/10 rounded-xl">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleCopy}
            className="px-3 py-1.5 text-sm bg-slate-800 hover:bg-slate-700 rounded-lg flex items-center gap-1.5 transition-colors"
          >
...
function IMemoMarkdownRenderer({ markdown }) {
  ...
  const flushTable = () => {
    if (tableBuffer.length < 2) {
      tableBuffer.forEach(l => elements.push(<p key={key++} className="text-slate-300 my-1">{l}</p>));
      tableBuffer = [];
      inTable = false;
      return;
    }
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-slate-700">
              {headers.map((h, i) => (
                <th key={i} className="text-left px-3 py-2 text-slate-300 font-semibold">{renderInline(h)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri} className="border-b border-slate-800 hover:bg-slate-800/30">
                {row.map((cell, ci) => (
                  <td key={ci} className="px-3 py-2 text-slate-400">{renderInline(cell)}</td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("# ")) {
      elements.push(<h1 key={key++} className="text-xl font-bold text-slate-100 mt-6 mb-3">{line.slice(2)}</h1>);
    } else if (line.startsWith("## ")) {
      elements.push(<h2 key={key++} className="text-lg font-semibold text-slate-100 mt-5 mb-2 border-b border-slate-800 pb-1">{line.slice(3)}</h2>);
    } else if (line.startsWith("### ")) {
      elements.push(<h3 key={key++} className="text-base font-semibold text-slate-200 mt-4 mb-1.5">{line.slice(4)}</h3>);
    } else if (line.startsWith("---")) {
      elements.push(<hr key={key++} className="border-slate-800 my-4" />);
    } else if (line.startsWith("> ")) {
      elements.push(
        <blockquote key={key++} className="border-l-4 border-slate-600 pl-4 my-2 text-slate-400 italic text-sm">
          {renderInline(line.slice(2))}
        </blockquote>
      );
    } else if (line.match(/^(\d+)\. /)) {
      elements.push(
        <p key={key++} className="text-slate-300 text-sm my-1 pl-2">{renderInline(line)}</p>
      );
    } else if (line.startsWith("- ")) {
      elements.push(
        <p key={key++} className="text-slate-300 text-sm my-0.5 pl-2">• {renderInline(line.slice(2))}</p>
      );
    } else if (line.trim() === "") {
      elements.push(<div key={key++} className="h-2" />);
    } else {
      elements.push(
        <p key={key++} className="text-slate-300 text-sm my-1 leading-relaxed">{renderInline(line)}</p>
      );
    }
  }
