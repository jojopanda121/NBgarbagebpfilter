...
router.post("/:taskId/messages", requireAuth, workspaceRateLimit, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (err?.status === 401 || err?.status === 403 || /认证失败|MINIMAX_API_KEY|KIMI_API_KEY|MOONSHOT_API_KEY/.test(msg)) {
      msg = `LLM 调用失败：${msg}（请检查服务端 .env 中的 MINIMAX_API_KEY、KIMI_API_KEY 或 MOONSHOT_API_KEY 是否有效）`;
    }
