...
- 每条问题 source_refs 必须引用 Fact Pack 的 F 或 C 编号,不得虚构编号。
- source_type=upload_structured 的 F 编号来自用户上传底层资料（财务表/客户清单/合同/Cap Table/合规材料），优先级最高。
- 看到 C 编号冲突、upload.red_flags、upload.claims_to_verify 或关键字段缺失时，必须衍生 financial / commercial / legal 类追问。
  ...
  inputSchema: {
    ...
    properties: {
      ...
      enable_bp_deep_parsing: {
        type: "boolean",
        description: "可选。兼容旧开关；上传材料结构化抽取会在上传后自动执行，并以 upload_structured F 编号用于财务稽核点抽取。",
      },
    },
    additionalProperties: false,
  },
