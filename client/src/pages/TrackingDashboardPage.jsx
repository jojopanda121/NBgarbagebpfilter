  ...
  return (
    <div className={`bg-gradient-to-br ${colors[color]} border rounded-xl p-4`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs text-[#4B5A72]">{label}</span>
      </div>
      <div className="text-2xl font-bold text-[#0D2145]">{value ?? "—"}</div>
      {sub && <div className="text-xs text-[#4B5A72] mt-1">{sub}</div>}
    </div>
  );
}
...
function StatusBadge({ status }) {
  const map = {
    ...
    unknown: { label: "未知", cls: "bg-slate-500/20 text-[#4B5A72]", icon: Clock },
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#0D2145]">训练数据看板</h1>
          <p className="text-sm text-[#4B5A72]">企业追踪 · 预测回测 · 数据导出</p>
        </div>
      </div>

      {/* Tab bar */}
      <div className="flex gap-1 bg-white border border-[#D8DCE8] rounded-xl p-1 mb-6 overflow-x-auto">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap ${
              tab === key
                ...
                : "text-[#4B5A72] hover:text-[#0D2145] hover:bg-white/5"
            }`}
          >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
          <h3 className="text-sm font-medium text-[#0D2145] mb-3">企业状态分布</h3>
            ...
            <div className="space-y-2">
              {Object.entries(status_distribution).map(([status, count]) => (
                <div key={status} className="flex items-center justify-between">
                  ...
                  <span className="text-[#0D2145] font-medium">{count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
          <h3 className="text-sm font-medium text-[#0D2145] mb-3">预测评级分布</h3>
          {Object.keys(prediction_accuracy.grade_distribution).length === 0 ? (
            <p className="text-sm text-[#8E9BB0]">暂无回测数据</p>
          ) : (
            <div className="space-y-2">
      ...
      {/* 最近快照 */}
      <div className="bg-white border border-[#D8DCE8] rounded-xl p-4">
        <h3 className="text-sm font-medium text-[#0D2145] mb-3">最近快照</h3>
        {recent_snapshots.length === 0 ? (
          <p className="text-sm text-[#8E9BB0]">暂无快照数据</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[#4B5A72] text-left">
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#1B4FD8] hover:bg-[#163069] disabled:opacity-50 rounded-lg text-sm text-[#0D2145] font-medium transition"
        >
          {runningQuarterly ? (
        ...
        <>
          <div className="bg-white border border-[#D8DCE8] rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[#4B5A72] text-left bg-[#EEF1F7]">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm">
              <span className="text-[#4B5A72]">共 {total} 家企业</span>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
          <div className="bg-white/5 rounded-lg p-3">
            <span className="text-[#4B5A72]">统一社会信用代码</span>
            <p className="text-[#0D2145] mt-1">{company.credit_code || "—"}</p>
          </div>
          <div className="bg-white/5 rounded-lg p-3">
            <span className="text-[#4B5A72]">创始人</span>
            <p className="text-[#0D2145] mt-1">{company.founder_names || "—"}</p>
          </div>
        </div>
...
function ExportTab() {
  ...
  return (
    <div className="max-w-lg">
      <div className="bg-white border border-[#D8DCE8] rounded-xl p-6 space-y-5">
        <div className="flex items-center gap-2 mb-2">
          <FileJson className="w-5 h-5 text-blue-400" />
          <h3 className="text-[#0D2145] font-medium">导出训练数据</h3>
        </div>
        ...
        <div>
          <label className="block text-sm text-[#4B5A72] mb-1">时间窗口</label>
          <select
            value={months}
            onChange={(e) => setMonths(Number(e.target.value))}
            className="w-full px-3 py-2 bg-[#EEF1F7] border border-[#D8DCE8] rounded-lg text-[#0D2145] text-sm focus:border-blue-500/50 focus:outline-none"
          >
...
function EmptyState({ message }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-[#8E9BB0]">
