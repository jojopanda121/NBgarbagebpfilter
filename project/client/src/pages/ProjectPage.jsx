...
export default function ProjectPage() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#F4F6FB] flex flex-col items-center justify-center">
        ...
        <button onClick={() => navigate("/app/history")} className="px-4 py-2 bg-[#E9EDF6] rounded-lg">
          返回历史记录
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F4F6FB]">
      {/* Header */}
      <header className="border-b border-[#CBD1E0] bg-[#F4F6FB]/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center">
              <Gavel className="w-5 h-5 text-[#0C1A30]" />
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleShare}
              disabled={sharing}
              className="px-3 py-1.5 text-sm bg-[#1749C9] hover:bg-[#0A1F3D] disabled:bg-[#DDE3F0] rounded-lg transition-colors flex items-center gap-1.5"
            >
            ...
            {displayScore != null && (
              <span className="px-2.5 py-1 rounded-lg bg-[#E9EDF6] text-sm font-bold text-[#0C1A30]">
                {displayScore} 分
                {project?.adjusted_score != null && project?.total_score != null && project.adjusted_score !== project.total_score && (
                  <span className="text-xs font-normal text-[#526078] ml-1">（已尽调）</span>
                )}
              </span>
            )}
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#2D3D54] truncate">{shareLink}</p>
            </div>
            <button
              onClick={handleCopyLink}
              className="shrink-0 px-3 py-1.5 bg-[#1749C9] hover:bg-[#0A1F3D] rounded-lg text-sm flex items-center gap-1.5"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <button
          ...
          className="flex items-center gap-2 text-[#2D3D54] hover:text-[#0C1A30] transition-colors mb-4"
        >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[#2D3D54] text-sm mt-1">
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex gap-1 border-b border-[#CBD1E0]">
          {tabs.map(tab => {
            const Icon = tab.icon;
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors -mb-px ${
                  activeTab === tab.key
                    ...
                    : "border-transparent text-[#526078] hover:text-[#0C1A30]"
                }`}
              >
            ...
            {rescoreResult && (
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                <p className="text-sm text-blue-400">
                  ...
                  <span className="ml-2 text-[#2D3D54]">
