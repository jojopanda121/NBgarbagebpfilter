...
export default function MessagesPage() {
      ...
      if (toUserId) {
        ...
        if (existing) {
          ...
          try {
            ...
            setDraftRecipient({ id: toUserId, name: p.profile?.name, user_type: p.profile?.user_type, avatar_url: p.profile?.avatar_url });
          } catch {
  ...
  if (!token) {
    return <div className="max-w-3xl mx-auto px-4 py-10"><RegistrationGate message="登录后查看与收发站内私信" /></div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-3 bg-white border border-[#D8DCE8] rounded-lg overflow-hidden" style={{ minHeight: 480 }}>
        {/* 会话列表 */}
        <aside className={`border-r border-[#EEF1F7] ${(activeId || draftRecipient) ? "hidden md:block" : ""}`}>
          <div className="px-3 py-2.5 text-sm font-semibold text-[#0D2145] border-b border-[#EEF1F7]">私信</div>
          {loadingList ? (
              ...
              {conversations.map((c) => (
                <li key={c.id}>
                  <button onClick={() => openConversation(c)}
                    ...
                    <Avatar src={c.counterpart?.avatar_url} name={c.counterpart?.name} id={c.counterpart?.id} size="md" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-medium text-[#0D2145] truncate">{c.counterpart?.name}</span>
                        {c.unread_count > 0 && <span className="ml-auto shrink-0 text-[10px] bg-[#1B4FD8] text-white rounded-full px-1.5">{c.unread_count}</span>}
                      </div>
                      <div className="text-xs text-[#8E9BB0] truncate">
                        {c.last_message ? (c.last_message.from_me ? "我: " : "") + c.last_message.body : "（暂无消息）"}
                      </div>
                    </div>
                  </button>
                </li>
        ...
        <section className={`flex flex-col ${(activeId || draftRecipient) ? "" : "hidden md:flex"}`}>
          {!header ? (
            <div className="flex-1 flex items-center justify-center text-sm text-[#8E9BB0]">选择左侧会话开始聊天</div>
          ) : (
            <>
              <div className="px-3 py-2.5 border-b border-[#EEF1F7] flex items-center gap-2">
                <button className="md:hidden text-[#8E9BB0]" onClick={() => { setActiveId(null); setDraftRecipient(null); setParams({}); }}>
                  <ArrowLeft className="w-4 h-4" />
                </button>
                <button onClick={() => header.id && navigate(`/forum/u/${header.id}`)} className="flex items-center gap-2 hover:opacity-80">
              ...
              <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2 bg-[#FAFBFC]" style={{ maxHeight: 420 }}>
                {loadingThread && !thread?.messages?.length ? (
                  <div className="py-10 flex justify-center"><Loader2 className="w-5 h-5 animate-spin text-[#8E9BB0]" /></div>
                ) : thread?.messages?.length === 0 ? (
                  <div className="text-xs text-[#8E9BB0] text-center py-10">还没有消息，发一句打个招呼吧</div>
                ) : (
                  thread.messages.map((m) => (
                    <div key={m.id} className={`flex ${m.from_me ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-sm whitespace-pre-line break-words ${
                        m.from_me ? "bg-[#1B4FD8] text-white rounded-br-sm" : "bg-white border border-[#E6E9F0] text-[#0D2145] rounded-bl-sm"
                      }`}>
                        {m.body}
                      </div>
                    </div>
