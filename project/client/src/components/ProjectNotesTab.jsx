...
const STAGE_CONFIG = {
  new:           { label: "新建",     color: "bg-slate-500/20 text-[#2D3D54] border-slate-500/30" },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-[#CBD1E0] rounded-xl p-4">
        <p className="text-sm font-medium text-[#0C1A30] mb-3">投资流程阶段</p>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-all ${
                  stage === s
                    ...
                    : "border-[#CBD1E0] bg-[#E9EDF6] text-[#526078] hover:border-[#A8B0C8]"
                }`}
              >
                {cfg.label}
              </button>
      ...
      {/* 标签 */}
      <div className="bg-white border border-[#CBD1E0] rounded-xl p-4">
        <p className="text-sm font-medium text-[#0C1A30] mb-3 flex items-center gap-1.5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {PRESET_TAGS.filter(t => !tags.includes(t)).map(t => (
            <button
              key={t}
              onClick={() => addTag(t)}
              className="px-2.5 py-1 rounded-full text-xs bg-[#E9EDF6] text-[#2D3D54] hover:bg-[#DDE3F0] border border-[#EEF1F7] transition-colors"
            >
              + {t}
            </button>
          ))}
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-1.5 bg-[#E9EDF6] border border-[#CBD1E0] rounded-lg text-sm text-[#0C1A30] placeholder:text-[#526078] focus:outline-none focus:border-blue-500"
          />
          <button
            onClick={() => addTag(tagInput)}
            disabled={!tagInput.trim()}
            className="px-3 py-1.5 text-sm bg-[#DDE3F0] hover:bg-slate-600 disabled:opacity-40 rounded-lg transition-colors"
          >
            添加
          </button>
        </div>
      ...
      <div className="bg-white border border-[#CBD1E0] rounded-xl p-4">
        ...
        <input
          ...
          className="px-3 py-2 bg-[#E9EDF6] border border-[#CBD1E0] rounded-lg text-sm text-[#0C1A30] focus:outline-none focus:border-blue-500"
        />
        {followupDate && (
          <button
            onClick={() => setFollowupDate("")}
            className="ml-2 text-sm text-[#526078] hover:text-[#0C1A30]"
          >
            清除
          </button>
        )}
      ...
      {/* 项目备注 */}
      <div className="bg-white border border-[#CBD1E0] rounded-xl p-4">
        <p className="text-sm font-medium text-[#0C1A30] mb-3">项目备注</p>
        <textarea
          value={notes}
          onChange={e => setNotes(e.target.value)}
          placeholder="记录你对这个项目的主观判断、关注点、下一步计划等...（支持 Markdown）"
          rows={8}
          className="w-full px-3 py-2.5 bg-[#E9EDF6] border border-[#CBD1E0] rounded-lg text-sm text-[#0C1A30] placeholder:text-[#526078] focus:outline-none focus:border-blue-500 resize-y leading-relaxed"
        />
      </div>
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="w-full py-2.5 bg-[#1749C9] hover:bg-[#0A1F3D] disabled:opacity-50 rounded-xl font-medium transition-colors flex items-center justify-center gap-2"
      >
        {saving
