...
function listPosts({ category, sort = "latest", page = 1, pageSize = 20, viewerId = null }) {
  ...
  switch (sort) {
    case "score":
      ...
      orderSql = "ORDER BY p.pinned DESC, p.score_total DESC NULLS LAST, p.created_at DESC";
      break;
    case "interest":
      // 感兴趣度：收到的撮合意向数
      orderSql = "ORDER BY p.pinned DESC, p.interest_count DESC, p.created_at DESC";
      break;
    case "hot":
