...
async function runAgentBWithBatching(extractedData, bpText, onProgress) {
    ...
    batches.map((batch, batchIdx) =>
      callLLM(
        ...
        if (!parsed) {
          ...
          return batch.map((c) => ({
            category: c.category, original_claim: c.claim, bp_claim: c.claim,
            ai_research: "核查批次解析失败", verdict: "存疑",
  ...
  // 层3: 精简模式
  if (!structuralResult || !structuralResult.validated_data) {
    console.warn("[B.2] 重试层2仍失败，启用兜底层3...");
...
async function runPipeline(bpText, onProgress) {
  ...
  return {
    ...
    search_summary: {
      ...
      queries_count: claimCount, provider: "minimax_builtin_knowledge",
    },
  };
}
