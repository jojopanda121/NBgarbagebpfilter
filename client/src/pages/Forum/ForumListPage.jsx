...
export default function ForumListPage() {
  ...
  const totalPages = data && !data.gated ? Math.ceil((data.total || 0) / (data.page_size || 20)) : 1;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-5">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16 text-[#8E9BB0]"><Loader2 className="w-5 h-5 animate-spin" /></div>
      ) : items.length === 0 ? (
        <div className="text-center py-16 text-sm text-[#8E9BB0]">还没有帖子，来发布第一个吧</div>
      ) : (
        <div className="space-y-2.5">
          {items.map((p) => <ForumPostCard key={p.id} post={p} />)}
        </div>
      )}
      ...
      {token && totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-5 text-sm">
          <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)}
            className="px-3 py-1.5 rounded-lg border border-[#D8DCE8] disabled:opacity-40">上一页</button>
          <span className="text-[#8E9BB0]">{page} / {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)}
            className="px-3 py-1.5 rounded-lg border border-[#D8DCE8] disabled:opacity-40">下一页</button>
        </div>
      )}

      <ForumDisclaimer variant="footer" />

      {showNew && <NewDiscussionModal onClose={() => setShowNew(false)} onCreated={() => { setShowNew(false); load(); }} />}
    </div>
  );
}
  ...
  return (
    <button onClick={onClick}
      className={`shrink-0 inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
        active ? "bg-[#1B4FD8] text-white" : "bg-white border border-[#D8DCE8] text-[#4B5A72] hover:bg-[#EEF1F7]"
      }`}>
      {children}
    </button>
  );
