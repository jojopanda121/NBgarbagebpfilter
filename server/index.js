...
*第三维度：商业验证与效率（满分100）*
- 参考核查报告中 category=financial 的核查结论，使用已验证的LTV/CAC/毛利率
- 单位经济模型基于核查后数据进行可持续性判断
...
【重要输出要求】只输出纯 JSON，不要任何 markdown 代码块或其他文字：
{
  ...
  "dimension_analysis": {
    ...
    "business_validation": {
      ...
      "label": "商业验证与效率",
      "subtitle": "商业模式 + 增长 + 财务",
      "finding": "单位经济验证：基于核查结论，LTV/CAC=XX（行业平均约为X-X），[合理/偏低/明显偏低]。毛利率：核查结论为实际约XX%，同类型公司（XX、XX）实际约XX-XX%，[基本合理/偏高/明显偏高]。",
      "bp_claim": "BP声称LTV/CAC=X，毛利率XX%",
      "ai_finding": "核查结论：同行业可比公司（XX/XX/XX）LTV/CAC实际约为X-X，毛利率约为XX-XX%..."
    },
    "team": {
      "score": 55,
      "label": "团队基因",
...
const CLAIM_BATCH_SIZE = 6; // 每批核查声明数，控制单次 token 量
...
async function runAgentBWithBatching(extractedData, bpText, sendSSE) {
  ...
  if (!structuralResult || !structuralResult.validated_data) {
    ...
    sendSSE({
      ...
      stage: "scoring_retry",
      percentage: 75,
      message: "评分结果解析失败，重试中...",
    });
    const retryPrompt =
      EXPERT_JUDGE_STRUCTURAL_PROMPT +
      "\n\n【紧急提醒】上次输出不是合法 JSON。这次只输出 JSON 对象，从 { 开始到 } 结束。";
    const retryRaw = await callLLM(retryPrompt, structuralInput, 16000);
    structuralResult = extractJson(retryRaw);
  }
