...
【BP 深度解析事实 — D-prefixed facts (本质是 BP 自报数据)】
- Fact Pack 中以 D 开头的事实是 schema-validated 数字（财务三表 / 单位经济 / 客户清单），confidence 字段反映可信度。
- **来源本质**：D 编号仍来自目标公司 BP，可信度 ≈ BP 自报，**低于外部检索**对竞品 / 公允数据的披露。
- 用法：把目标公司的 D 编号数字与**外部检索（F 编号 source_type=external_search）**抓到的竞品融资 / 估值 / 客户规模做对照；优先引用外部检索作为竞品事实基础，再用 D 编号给目标公司定位。
- 不要把 D 编号当成竞品的事实依据；竞品行的数字必须来自外部检索或确认竞品材料。
  ...
  inputSchema: {
    ...
    properties: {
      ...
      enable_bp_deep_parsing: {
        type: "boolean",
        description: "可选。开启后并行跑 3 个 BP 深度解析 agent 把财务三表/单位经济/客户清单结构化注入 Fact Pack (D-prefixed)。默认走 env ENABLE_BP_DEEP_PARSING。",
      },
    },
    additionalProperties: false,
  },
