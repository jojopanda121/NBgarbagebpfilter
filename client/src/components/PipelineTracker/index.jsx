...
const PipelineTracker = memo(function PipelineTracker() {
  ...
  return (
    <div className="mt-8 space-y-4">
      {/* ── 总体进度条 ── */}
      <div className="bg-white/90 border border-[#D8DCE8]/50 rounded-xl p-4 space-y-3">
        {/* 顶行：阶段描述 + 百分比 */}
        ...
        <div className="flex items-center justify-end gap-1.5 h-4">
          {etaText && (
            <>
              <Clock className="w-3.5 h-3.5 text-[#4B5A72] shrink-0" />
              <span className="text-xs text-[#4B5A72] tabular-nums">
                预计剩余&nbsp;{etaText}
              </span>
            </>
          )}
      ...
      <div className="flex items-start gap-2.5 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
        ...
        <p className="text-xs text-[#4B5A72] leading-relaxed">
          ...
          <strong className="text-[#1B4FD8] font-semibold underline decoration-[#1B4FD8]/40 underline-offset-2 mx-1">历史报告</strong>
          中查看结果。
        </p>
      </div>
          ...
          >
            ...
            >
              {done ? (
                ...
                <Icon className="w-5 h-5 text-[#8E9BB0]" />
              )}
            </div>
            <span
    ...
    <div className="mt-8">
      <div className="p-5 bg-amber-500/10 border border-amber-500/30 rounded-xl space-y-3">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={reset}
            className="px-4 py-2 bg-[#EEF1F7] hover:bg-[#E5E9F4] border border-[#D8DCE8] rounded-lg text-sm text-[#0F1C36] transition-colors"
          >
            继续其他分析
          </button>
        </div>
