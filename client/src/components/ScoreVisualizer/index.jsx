...
const RadarChartPanel = memo(function RadarChartPanel({ dimensions }) {
  if (!dimensions) {
    return <p className="text-[#8E9BB0] text-sm">暂无数据</p>;
  }
...
const ValuationThermometer = memo(function ValuationThermometer({ data }) {
  if (!data || (!data.bp_multiple && !data.industry_avg_multiple)) {
    return (
      <div className="flex flex-col items-center justify-center h-32 text-[#8E9BB0] text-sm">
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          <span className="text-[#4B5A72]">BP 声称估值倍数</span>
          <span className="font-mono font-bold text-orange-400">{bp}x</span>
        </div>
        <div className="h-4 bg-[#EEF1F7] rounded-full overflow-hidden">
          <div
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          <span className="text-[#4B5A72]">
            行业平均估值倍数
            {data.industry_name && (
              <span className="text-[#8E9BB0] ml-1">
                ({data.industry_name})
              </span>
            )}
          </span>
      ...
      >
        <span className="text-sm text-[#4B5A72]">溢价程度：</span>
        <span
      ...
      {data.data_source && (
        <p className="text-xs text-[#8E9BB0] text-center">数据来源：{data.data_source}</p>
      )}
      {data.analysis && (
        <p className="text-xs text-[#8E9BB0] leading-relaxed">{data.analysis}</p>
      )}
    </div>
  );
});
        ...
        if (key === "external_risk" && dim.multiplier !== undefined) {
          return (
            <div key={key} className="flex items-center gap-3">
              <Icon className="w-4 h-4 text-[#4B5A72] shrink-0" />
              <span className="text-sm text-[#4B5A72] w-20 sm:w-32 shrink-0 truncate">
                {dim.label || key}
              </span>
              <div className="flex-1 flex items-center gap-2">
                <span className="text-sm text-[#8E9BB0]">乘数效果:</span>
                <span
