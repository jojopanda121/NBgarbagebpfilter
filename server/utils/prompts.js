...
**缺数据 ≠ 不诚信（强制）：**
- "证伪"只适用于 BP **主动做出**、且被证据**证明为假**的具体声明（技术不可行/市场不存在/数字明显错误）。
- BP **未披露 / 没有某类数据**（如"BP 没有财务数据""未给收入/毛利"）本身不是一条声明，**不得**判为
  证伪/夸大/信息不对称——信息不对称仅在你有"同行普遍披露、它刻意回避负面"的证据时才用。
- **禁止**用规范性理由（"行业标准做法应包含 X""BP 应当披露 Y"）去给缺失项判负。缺数据 → 判"存疑"
  （中性及格，只降覆盖率），大部分早期 BP 本就不写真实财务，这不是项目的诚信问题。
- 财务/市场**预测**：尽量与你研究的真实基准对比——远超合理基准才判"夸大/严重夸大"（前瞻预测会被列入
  尽调红旗、不据此一票否决）；若 BP 根本没有预测/数据可对照，判"存疑"，不扣诚信。
...
【harness 强约束 — 不要只报一个数，要给"带证据分层"的拆解】
为了让 JS 端能复算、压制 BP 自报注水，除了上面两个整数，还必须额外输出：

1) TRL_Evidence —— TRL 证据阶梯（逐级标注证据强度，不是只报终值）
   - bp_claimed_trl：BP 自己声称/暗示的 TRL 级别（看不出就给 null）
   - ladder：对关键里程碑逐条评估，每条 { level(1-9), status, evidence }
     status 三档（**这是核心：必须如实区分"有证据"和"BP自吹"**）：
       · verified —— 有第三方检索证据 / 上传材料（检测报告、交付验收、量产证据、客户实装）支撑
       · claimed  —— 仅 BP 自报、无可核实证据
       · absent   —— 该级未达成或无任何信息
     evidence：≤80字写明依据或缺口。**禁止把仅 BP 自报的级别标成 verified。**
   TRL 阶梯参考：L1-2 原理/概念，L3-4 实验室验证，L5-6 相关环境/中试，L7 真实环境/试点客户，L8-9 鉴定/首批商用/量产。

2) Moat_Rubric —— 护城河拆成 4 个子因子（供应链咽喉由独立 skill 另算，这里不重复）
   每个子因子 { score(0-100), evidence_tier(verified/claimed/absent), note(≤60字) }：
     · differentiation     差异化/IP/独家数据：专利权利要求、独占数据、难复制的工程 know-how
     · switching_cost      转换成本/客户锁定：迁移代价、深度集成、长约
     · traction_position   已验证的客户/落地相对竞品的位置（以核查过的事实为准，非BP自报）
     · competitive_density 竞争密度：检索到的竞品数量与强度，越不拥挤/越占位给越高分
...
【重要输出要求】只输出纯 JSON，不要包含 dimension_analysis：
{
  ...
  "validated_data": {
    ...
    "TRL_Evidence": {
      "bp_claimed_trl": 8,
      "ladder": [
        { "level": 4, "status": "verified", "evidence": "已有实验室原型，第三方报道证实" },
        { "level": 7, "status": "claimed", "evidence": "BP 称已试点客户，但无可核实交付证据" },
        { "level": 8, "status": "absent", "evidence": "未见量产/鉴定证据" }
      ]
    },
    "Moat_Rubric": {
      "differentiation":     { "score": 70, "evidence_tier": "verified", "note": "持有3项核心专利" },
      "switching_cost":      { "score": 55, "evidence_tier": "claimed",  "note": "BP称深度集成，待核实" },
      "traction_position":   { "score": 60, "evidence_tier": "verified", "note": "已签2家标杆客户" },
      "competitive_density": { "score": 50, "evidence_tier": "verified", "note": "检索到约6家同类竞品" }
    },
...
5. **web_search**（**本任务必须至少调用一次**）：用于补充行业 TAM/CAGR/政策口径/Top 玩家/竞品融资轮次等市场事实。检索后**凝练为事实**，禁止整段贴。**未先调用 web_search 即输出 JSON 视为违规。**
...
{
  "company_name": "公司全称",
  "headline": "≤36字。**第三方投资视角的 thesis**，不是 BP slogan。示例：'技术已验证，商业化仍需核实'",
  "company_overview": {
    "summary": "≤140字。**客观描述**公司在做什么、为谁解决什么、当前阶段、关键里程碑。剔除 BP 自夸用词。",
    "products": [
      { "name": "产品/业务名≤12字", "desc": "≤48字客观描述（含已验证状态、核心客户/合作样例）" }
    ]
  },
  "market_opportunity": {
    "kpis": [
      { "label": "TAM",   "value": "如 800 亿 或 暂无（必要时通过 web_search 核实，注明口径年份）" },
      { "label": "CAGR",  "value": "如 28% 或 暂无" },
      { "label": "渗透率","value": "如 6%, 早期 或 暂无" },
      { "label": "增量空间","value": "如 5 年内翻 3 倍 或 暂无" }
    ],
    "drivers": [
      { "type": "政策", "text": "≤55字，必须引用具体政策名/发布时间/部委，必要时 web_search 核实。无可核实时填 '暂无明确政策催化'。" },
      { "type": "技术", "text": "≤55字" },
      { "type": "需求", "text": "≤55字" }
    ],
    "competition": "≤80字。Top 玩家 + 公司**实际**排位 + 与头部的差异/差距。无法判断时写'公司位次待验证'。"
  },
  "highlights": [
    { "title": "≤14字标题（**投研视角**，非营销标题）", "desc": "≤56字佐证，可写多个事实/数字/客户名，**必须**能在输入数据或 web_search 结果中找到。如无客观正面证据，写'暂无突出亮点'。" }
  ],
  "risks": [
    { "title": "≤10字风险标题", "desc": "≤50字客观描述，优先来自 verdict.risk_flags 与 claim_verdicts，可点名缺口与影响面。" }
  ],
  "deal_breakers": [
    { "title": "≤14字致命伤标题", "logic": "≤80字 为什么不解决就杀掉交易；必须能引用具体 claim_verdict 或 deep_research 结论，不要写'市场风险大'空话。", "falsification_test": "≤50字 做什么尽调能洗清这一条（如'拿到前 5 大客户合同核实集中度≤35%'）。不能写'继续观察'。" }
  ],
  "traction_snippet": {
    "comment": "经营进度速览 — **4 个字段固定为 ARR/MoM/NDR/Burn，跨行业不变**。来源优先级 financial_agent / claim_verdicts > extracted_data > user_overrides。所有值都必须带单位和口径年份/月份（如 ARR='¥1.2 亿 (2025Q4)'、MoM='+8% (2026 年 3 月)'）。**禁止编造**——硬科技/医疗等没有 ARR 的项目，按用户在 user_overrides 中给的等价指标填，否则填 '未披露'。",
    "arr": "Annual Recurring Revenue，统一换算 ¥/$，必须带口径年份/季度。无填'未披露'。",
    "mom": "月环比增长率 (Month-over-Month)，百分比 + 月份，如 '+12% (2026 年 3 月)'。无填'未披露'。",
    "ndr": "Net Dollar Retention，百分比 + 计算期，如 '118% (TTM)'。SaaS 项目必填。非订阅业务可填'N/A — 非订阅模式'。",
    "burn_rate": "月均烧钱率 + 跑道，如 '¥800 万/月，跑道 14 个月'。盈利项目填'已盈利 (2025 YTD)'。无填'未披露'。"
  },
  "deal_dynamics": {
    "round": "本轮轮次。来源优先级 user_overrides.funding_round > extracted_data.funding_round > web_search 公开报道。无可核实数据填'未披露'。",
    "amount": "本轮融资金额（如 '¥1.2 亿' / 'US$25M'）。来源同上。无填'未披露'。",
    "valuation": "估值预期（如 'Pre-money ¥8 亿' / 'Post-money US$120M'）。**必须标注 Pre/Post**，无法判定写'估值口径未披露'；BP 与 user_overrides 矛盾时以 user_overrides 为准。",
    "lead_investor": "领投方/领投进度（如 '红杉中国 领投确认' / '领投未定，已有 2 家跟投 TS' / '暂无领投意向'）。来源 user_overrides > extracted_data > web_search。",
    "progress": "本轮进度与时点（如 '已签 TS，预计 Q1 close' / '路演中，预计 4 周关闭' / '前期接触'）。需明确时间或里程碑，模糊措辞填'进度未披露'。"
  },
  "footer": {
    "founded": "成立年份 或 暂无",
    "team_size": "团队规模 或 暂无",
...
2a. **deal_dynamics 5 个字段为必填**：round / amount / valuation / lead_investor / progress。无可核实信息时统一写"未披露"（**不得**写"暂无"或留空），方便投资人一眼识别 data room 缺口；**禁止**编造领投方、估值数字或 close 时间。
2b. **traction_snippet 4 个字段为必填**：arr / mom / ndr / burn_rate。即使是硬科技/医疗等非 SaaS 项目也要给这 4 项——若指标不适用就明确写 "N/A — <理由>"（如 'N/A — 临床前期无收入'），若数据未披露就写"未披露"，**禁止**用 TAM/CAGR/临床阶段等市场指标替换；这一格是"项目自身经营脉搏"，与 market_opportunity.kpis 的"市场容量"严格分开。
2c. **deal_breakers 0-2 条，可选但强烈建议**：与 risks 严格分开——risks 是"需要监控的"，deal_breakers 是"不解决就杀掉这笔交易"。每条必须**可证伪**（falsification_test 是具体动作如"拿合同核实集中度"，不是"继续观察"）。真没看到 deal-breaker 信号就**留空数组**，**不要凑数**；强行编一条 deal-breaker 视为输出不合格。
...
6. **必须先调用 web_search** 至少一次，把 TAM/CAGR/政策/Top 玩家/竞品融资轮次等行业事实拉回来，再写 market_opportunity 与 competition；只引官方/主流财经媒体/上市公司公告，不引自媒体。
7. 严格只输出 JSON 对象。`;
